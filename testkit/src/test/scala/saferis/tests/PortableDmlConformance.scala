...
/** DDL and DML every database Saferis ships has to agree on, rendered by the dialect under test. Tables come from
  * `createTable`, so the dialect's column types are part of what is checked.
  */
  ...
  def conformance(using Dialect) =
    suite("portable DML")(
      ...
      test("pagedStream pages through every row"):
        ...
        yield assertTrue(pages.map(_.items.size) == Chunk(2, 1), pages.flatMap(_.items).map(_.id) == Chunk(1, 2, 3))
      ,
