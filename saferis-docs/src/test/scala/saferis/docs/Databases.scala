...
- **Tables created by Saferis 0.19** declared booleans as `integer`, every float and decimal as `real`, and dates and timestamps as `text`. On those columns a boolean or date/time field fails to decode, and a `real` column reads as a 4-byte `Float`, so a `Double` stored there loses precision. Recreate those tables from the current DDL.
